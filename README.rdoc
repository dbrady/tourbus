...
In December of 2010, it was reworked to change the data model. Now it
uses simpler thread&dispatch model, and has a tweaked data model. This
allows it to represent other workflows.

== Motivation

TourBus was written for flexibility and scalability in a website
testing tool. The existing tools tend to provide one, but not the
other. Selenium is ultraflexible but limited to the number of browsers
you can have open at once, while Apache Bench is powerful and fast but
...
It's worth taking a few minutes to understand the TourBus abstraction
model. It'll help understand the various things.

=== Starting from the Middle

The main piece is a `Tourist`, this represents a common web visitor's
workflow. 

Moving downwards, a `Tourist` goes on a series of `Tours`, and timing data
is collected per tour. A `Tour` may have multiple things inside, but
we don't talk about those.

Moving upwards, there are `Guides`. These control how many `Tourists`
are operating concurrently. A Guide probabilistically picks a Tourist,
takes them on their chosen Tours, reports timing data back to the
TourBus, and then repeats. 

=== Model

TourBus (stats are stored here)
 -> Guide (one per concurrent thread)
     -> Tourist (has a scheduled list of tours)
         -> tours

=== Concurrency

We want to simulate N concurrent users, each operating independantly. 

== Results

What good is testing without results? As this is designed to be run
thousands of time, there's a lot of data. Right now, we're just
storing data and leaving the presentation to other tools.

Since everyone loves nosql, and that allows us to skip on schema
definitions, let's go with that. There are several different messages.

(runid is the time the run started. It's unique enough for us)

The main logs:

runid, concurrancy, tourist_type, 
  tour_num, tour_name, start_time, end_time, elapsed_time, status, messages

runid, concurrancy, tourist_type, 
  "total", start_time, end_time, elapsed_time, status

We might add an additional thread to note machine load. Since everyone
infrastructure is different, this is mostly left as an exercise to the
reader. But the log format might look like:

runid, concurrancy, "monitoring", "db", "load", time, loadavg
...
* Make a folder called tourists and put a file in it called simple.rb. In
  it write:
...
* Files in ./tourists should have classes that match their names. E.g.
  "class BigHairyTest < Tourist" belongs in ./tourists/big_hairy_test.rb

* Think Test::Unit. tour_* methods will be found automagically.
  setup() and teardown() methods will be executed at the appropriate
  times.

=== Example TourBus Run

You want to invoke +tourbus+ from the parent directory of the @tourists/@ folder.
...
== Developing Tourists with webrat

* http://webrat.rubyforge.org/
