...
module Output
  ...
  #  load_default_class_file(self, const)
  #end
  ...
  def self.load(file_format, *args)
    ...
    klass = nil
    if file_format.kind_of?(Output::Base)
      # this already is a file format! return itself
      return file_format

    elsif file_format.kind_of?(Class) && file_format.ancestors.include?(Output::Base)
      ...
      require file_format
      const = to_camelcase(File.basename(file_format, '.rb'))
      if FileFormat.const_defined?(const)
        klass = Output.const_get(const)
      ...
      klass = Output.const_get(to_camelcase(file_format))
    end
    ...
    raise "Invalid FileFormat class" unless klass.kind_of?(Class) && klass.ancestors.include?(Output::Base)
